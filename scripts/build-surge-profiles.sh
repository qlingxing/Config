...
MANAGED_CONFIG_BASE_URL=${SURGE_MANAGED_CONFIG_BASE_URL:-https://raw.githubusercontent.com/qlingxing/Config/main/Surge}
...
build_profile() {
  ...
  managed_url="$MANAGED_CONFIG_BASE_URL/$target_relative_path"

  mkdir -p "$(dirname "$target")"

  {
    printf '#!MANAGED-CONFIG %s interval=86400 strict=false\n' "$managed_url"
    printf '%s\n' "# Generated from source/surge. Do not edit this file in the repository."
    printf '%s\n' "# $title"
    printf '%s\n' '# Keep device-specific nodes in a linked profile so managed updates cannot overwrite them.'
    printf '\n'
