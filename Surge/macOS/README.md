...
Mac 6 profile also enables the web dashboard. Both Raw entries import as
read-only managed sources. Use an editable copy for the locally generated MITM
certificate and manual nodes. Before Sub-Store produces a node, the copy remains
...
After creating the editable copy and its CA, install Sub-Store from Surge's
Modules screen. Its production URL for the `All` collection is already active
in the corresponding node-source group. The main configuration cannot install
